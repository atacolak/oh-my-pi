...
- after commit, the next author does **not** start until residual context grows by the speculation lead from that compact's `tokensAfter`; a checkpoint-bound residual is treated as headroom so stock shake does not eat the raw continuation
