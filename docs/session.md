...
| `user_todo_edit`         | `{ phases: TodoPhase[] }`, where `TodoPhase` is `{ name, tasks, kind?: "continuing" \| "passive" }`                                                                                                                                                      | SDK/UI todo editing persists the complete phase snapshot. Todo restoration scans backward for the latest snapshot (or a successful `todo` tool result) and restores its phases, including each phase's optional `kind`, so the field survives the backward branch scan after compaction or resume. Omission means continuing. |
