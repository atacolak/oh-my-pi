...
- Hot Handoff waits leftover author time at the commit threshold. Checkpoint-bound `firstKeptEntryId` uses the stock `keepRecentTokens` cut so the raw window is recent exact context and may overlap the Handoff Document. If that native cut would skip post-snapshot entries the semantic author never saw, it falls back to the first entry after the checkpoint. An oversized post-checkpoint tail is recut once through the current leaf; recut failure keeps the original armed result. After commit, the next author waits until residual context grows by the speculation lead. Checkpoint-bound residuals skip stock shake/prune so the exact raw window is not stubbed into artifact placeholders. An unresolved `session.compacting` hook is held conservatively; once it resolves without a Hot Handoff model, stock prune/shake resume.
