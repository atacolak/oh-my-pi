...
async function autoStart(ctx: InteractiveModeContext): Promise<boolean> {
	ctx.collabController.autoStart();
	await ctx.collabController.idle();
	return ctx.collabController.host !== undefined;
}
...
describe("collab auto-start", () => {
	it("is off by default", async () => {
		const ctx = context();
		await expect(autoStart(ctx)).resolves.toBe(false);
	...
	it("starts on an explicit local relay, writes the full link, and avoids QR/link output", async () => {
		...
		try {
			await expect(autoStart(ctx)).resolves.toBe(true);
			...
			await ctx.collabController.shutdown("test done");
	...
	it("serializes concurrent host starts into one relay connection", async () => {
		...
		try {
			const first = ctx.collabController.start({ access: "control", relay: "ws://localhost:8787" });
			const second = ctx.collabController.start({ access: "control", relay: "ws://localhost:8787" });
			await waitFor(() => start.mock.calls.length === 1, "first host start");
	...
	it("blocks guest joins while host startup owns the collab role", async () => {
		...
		const start = spyOn(CollabHost.prototype, "start").mockImplementation(async () => gate.promise);
		try {
			const pending = ctx.collabController.start({ access: "control", relay: "ws://localhost:8787" });
			await waitFor(() => ctx.collabHost instanceof CollabHost, "in-flight host");
			expect(ctx.collabController.host).toBeDefined();
			gate.resolve();
			await pending;
		} finally {
			await ctx.collabController.shutdown("test done");
	...
	it("tears down a completed host if a guest appears during startup", async () => {
		...
		try {
			const pending = ctx.collabController.start({ access: "control", relay: "ws://localhost:8787" });
			await waitFor(() => start.mock.calls.length === 1, "host start in flight");
	...
	it("cancels a pending host handshake from stop before the host attaches", async () => {
		...
		try {
			const pending = ctx.collabController.start({ access: "control", relay: "ws://localhost:8787" });
			pending.catch(() => {});
			await waitFor(() => ctx.collabHost instanceof CollabHost, "host installed");
			await ctx.collabController.stop("host stopped");
			await expect(pending).rejects.toBeInstanceOf(CollabHostStoppedError);
			expect(ctx.collabHost).toBeUndefined();
		} finally {
			connect.mockRestore();
		}
	...
	it("closes the relay socket when stop races a completed handshake", async () => {
		...
		const connect = spyOn(CollabSocket.prototype, "connect").mockImplementation(function (this: CollabSocket) {
			...
			this.onOpen = () => {
				previous?.();
				void ctx.collabController.stop("host stopped");
			};
			originalConnect.call(this);
		});
		try {
			await expect(
				ctx.collabController.start({ access: "control", relay: "ws://localhost:8787" }),
			).rejects.toBeInstanceOf(CollabHostStoppedError);
	...
	it("does not attach or write a link after a cancelled pending start", async () => {
		...
		try {
			...
			ctx.collabController.autoStart();
			await waitFor(() => ctx.collabHost instanceof CollabHost, "auto-start host");
			await ctx.collabController.stop("host stopped");
			await ctx.collabController.idle();
			expect(ctx.collabHost).toBeUndefined();
	...
	it("stops an attached host from session teardown before dispose", async () => {
		installInMemoryRelay();
		const ctx = context({ "collab.autoStart": "control", "collab.relayUrl": "ws://localhost:8787" });
		await expect(autoStart(ctx)).resolves.toBe(true);
		expect(ctx.collabHost).toBeInstanceOf(CollabHost);
		await ctx.collabController.shutdown("session shutdown");
		expect(ctx.collabHost).toBeUndefined();
	});
	...
	it("refuses auto-start from a project-local redirected global config", async () => {
		...
		try {
			...
			expect(warnings.join(" ")).toContain("outside config overlays");
		} finally {
			owned.mockRestore();
			start.mockRestore();
			await fs.rm(dir, { recursive: true, force: true });
	...
	it("trusts auto-start from an argv-selected named profile after a project agent dir redirect", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "omp-collab-auto-"));
		const projectDir = path.join(dir, "project");
		...
		const fromArgv = spyOn(dirs, "isProfileSelectedFromArgv").mockReturnValue(true);
	...
	it("trusts auto-start from a parent OMP_PROFILE even when project dotenv owns PI_PROFILE", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "omp-collab-auto-"));
		const projectDir = path.join(dir, "project");
		const agentDir = path.join(dir, "profiles", "work", "agent");
		await fs.mkdir(agentDir, { recursive: true });
	...
	it("still refuses a project agent dir redirect under an argv-selected default profile", async () => {
		...
		const fromArgv = spyOn(dirs, "isProfileSelectedFromArgv").mockReturnValue(true);
		const profile = spyOn(dirs, "getActiveProfile").mockReturnValue(undefined);
		try {
			expect(settings.getProvenance("collab.autoStart")).toBe("global");
			await expect(autoStart(ctx)).resolves.toBe(false);
			expect(start).not.toHaveBeenCalled();
	...
	it("still refuses a project config dir redirect under an argv-selected named profile", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "omp-collab-auto-"));
		const projectDir = path.join(dir, "project");
		const agentDir = path.join(projectDir, "attacker-config", "agent");
		const target = path.join(dir, "sensitive");
	...
	it("honors a project auto-start opt-out under an overlay re-enablement", async () => {
		...
		const start = spyOn(CollabHost.prototype, "start");
		try {
			expect(settings.get("collab.autoStart")).toBe("control");
			expect(settings.getProvenance("collab.autoStart")).toBe("overlay");
			await expect(autoStart(ctx)).resolves.toBe(false);
			expect(start).not.toHaveBeenCalled();
			expect(ctx.collabHost).toBeUndefined();
	...
	it("does not attach a host that closed fatally before start returned", async () => {
		...
		await expect(autoStart(ctx)).resolves.toBe(false);
		expect(ctx.collabHost).toBeUndefined();
		expect(status.some(text => text.includes("Collab auto-start failed"))).toBe(true);
		expect(errors).toEqual([]);
		await ctx.collabController.stop("host stopped");
		expect(ctx.collabHost).toBeUndefined();
	});
	...
	it("refuses the implicit public relay", async () => {
		const warnings: string[] = [];
		const ctx = context({ "collab.autoStart": "control", showWarning: (text: string) => warnings.push(text) });
		await expect(autoStart(ctx)).resolves.toBe(false);
		expect(ctx.collabHost).toBeUndefined();
	...
	it("allows an explicitly configured public relay", async () => {
		installInMemoryRelay();
		const ctx = context({ "collab.autoStart": "control", "collab.relayUrl": DEFAULT_RELAY_URL });
	...
	it("stops an attached host before write-link publication finishes", async () => {
		...
		try {
			...
			expect(ctx.collabHost).toBeInstanceOf(CollabHost);
			const stopping = ctx.collabController.stop("host stopped");
			await Promise.resolve();
	...
	it("sanitizes auto-start failures before showing them", async () => {
		...
		expect(status[0]).toContain("Collab auto-start failed");
		expect(status[0]).toContain("Invalid relay URL");
		expect(status[0]).toContain("~/secret");
		expect(status[0]).not.toContain(home);
		expect(status[0]).not.toContain("\u001b");
		expect(status[0]).not.toMatch(/[\t\n]/);
	});
	...
	it("skips guests and an existing host", async () => {
		const guest = context({ "collab.autoStart": "control", collabGuest: {} });
		expect(await autoStart(guest)).toBe(false);
		const host = context({ "collab.autoStart": "control", collabHost: {} });
		expect(await autoStart(host)).toBe(false);
	});
});
