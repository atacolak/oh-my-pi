...
describe("terminal notifications", () => {
	...
	it("claims Herdr delivery without spawning a session-title card", () => {
		Bun.env.HERDR_ENV = "1";
		...
		// Herdr already toasts Idle/Blocked from pane.report_agent. Claiming
		// delivery without `herdr notification show` keeps OSC 9/99 from firing
		// (Herdr swallows those) and drops the duplicate session-title card.
		expect(spawn).not.toHaveBeenCalled();
		expect(stdout).not.toHaveBeenCalled();
	});

	it("claims delivery when a launcher dropped HERDR_ENV but kept the pane id", () => {
		delete Bun.env.HERDR_ENV;
		Bun.env.HERDR_PANE_ID = "w6:p1";
		const stdout = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
		const spawn = vi.spyOn(Bun, "spawn").mockImplementation((..._args: unknown[]) => ({ unref: vi.fn() }) as never);
	...
	it("skips the session-title spawn for done, request, and none", () => {
		Bun.env.HERDR_ENV = "1";
		Bun.env.HERDR_PANE_ID = "w6:p1";
		const stdout = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
		const spawn = vi.spyOn(Bun, "spawn").mockImplementation((..._args: unknown[]) => ({ unref: vi.fn() }) as never);
	...
	it("claims the Herdr pane over the outer cmux surface", () => {
		...
		// so it must win over the container — without spawning either CLI.
		Bun.env.HERDR_ENV = "1";
		Bun.env.HERDR_PANE_ID = "w6:p1";
		Bun.env.CMUX_SURFACE_ID = "123e4567-e89b-12d3-a456-426614174000";
	...
	it("does not fall through to OSC when herdr would be missing", () => {
		Bun.env.HERDR_ENV = "1";
		Bun.env.HERDR_PANE_ID = "w6:p1";
		mutableTerminal.notifyProtocol = NotifyProtocol.Osc99;
		const writes: string[] = [];
